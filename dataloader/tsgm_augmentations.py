...
"""
    这里我们定义了一些时间序列数据的增强方法, 用于扩充原始数据. 
    这部分代码参考了tsgm库的实现, 由于tsgm库是基于tensorflow的, 这里将其重构了一下, 使其可以适配numpy. 
    文档: https://tsgm.readthedocs.io/en/latest/
    源码: https://github.com/AlexanderVNikitin/tsgm/blob/main/tsgm/models/augmentations.py
...
class BaseAugmenter:
    ...
    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None, n_samples: int = 1) -> AugmentationOutput:
        """
            依据输入的数据集 X, 生成 n_samples 个增强样本. 
            如果输入数据集包含标签, 则返回的增强样本也包含标签. 
            所有数据都是numpy数组. 
        """
...
class BaseCompose:
    def __init__(
        ...
        """
            augmentations: 一个增强方法列表, 每个元素都是一个 BaseAugmenter 类的实例. 
        """
